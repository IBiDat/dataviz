...
[![](Polio Vaccine - WSJ.png){.external width="100%"}](http://graphics.wsj.com/infectious-diseases-and-vaccines/)

The visualization was so good that it had a great impact on social networks...

[![](twitterbg.png){.external width="100%"}](https://twitter.com/BillGates/status/568749655112228865)
 ...
 p <-  p + labs(x="", y="", fill="")+
  ...
  theme(aspect.ratio = 6.5/12.5,
    ...
    plot.margin=grid::unit(c(.5,.5,1.5,.5), "cm"),
        axis.text.y=element_text(size=4.5, family="Tajawal", 
                                 hjust=1),
...
p1 <- Polio_sum %>% ggplot() +
  ...
  theme(aspect.ratio = 6.5/12.5,
    ...
    plot.margin=grid::unit(c(.5,.5,1.5,.5), "cm"),
        axis.text.y=element_text(size=3.5, family="Tajawal"),
        axis.text.x=element_text(size=8),
        axis.ticks.y=element_blank(),
    axis.ticks.x=element_line(color = "grey50", size = 0.5),
    axis.line.x =element_line(color = "grey50", size = 0.5),
...
First I want to change the palette color to a color with more visual impact. That is why I want to choose a palette that makes it easier to see the years with the highest polio cases. That's why I chose the YlGnBu palette from the RColorBrewer package, which gives a very interesting effect.
...
p1 <- p1 + geom_segment(x=as.Date("1955-01-01"), xend=as.Date("1955-01-01"), 
  ...
  annotate("text", x=as.Date("1953-01-01"), y=56, label="First vaccine", 
...
p1 <- p1 + labs(title="Effect of the introduction of the two polio vaccines in the US",
       subtitle="The heat map represents a visualization of the positive cases of Polio in the USA during the years 1928 and 1971 for the three existing virus \ntypes (Nonparalytic poliomyelitis, Paralytic poliomyelitis, poliomyelitis), where the decrease of positive cases due to the introduction of the \ntwo vaccines can be clearly seen.")+
...
```{r}
Casesxstate <- Polio_sum %>%  group_by(State) %>% summarise(total_cases = sum(sum_cases))
...
casebars <- ggplot(Casesxstate) + 
  aes(x=total_cases, y=fct_rev(State), fill=total_cases)+
...
ggsave("resultadofinalmejorado.pdf", plot = p2, width = 20, height = 20, units = "cm")
