...
clean_data <- data %>% 
  ...
  # PLACE: Create 3 columns, one for each of the Urban/Suburban/Rural values of the usr column
  ...
  # AGE: Create 4 columns, one for each of the age ranges of the graph (from age column)
  ...
  # EDUCATION: Create 3 columns, one for each of the education ranges of the graph (from educ2 column)
...
clean_data <- clean_data %>%
  ...
  summarise(
    ...
    # GENDER: Create 2 columns, one for each of the gender categories of the graph (from gender column)
    Men      = perc(weight, YesNo==1, gender==1),
    Women    = perc(weight, YesNo==1, gender==2),
    # RACE: Create 3 columns, one for each of the race groups of the graph (from racecmb and hisp columns)
    ...
    # INCOME: Create 4 columns, one for each of the income ranges of the graph (from income column)
...
and numbers at the beginning ("X18_19", "HS_or_less"...). Also, we can order now
the name of the platforms according to their value for variable "total" (descending
order):
...
rep <- clean_data %>%
  ...
  # Collecting columns (demographic subgroups, excluding platform column) as one column called "key"
...
### Facetting
We will use column "group" to stablish the facets. For this, we create another
...
only the 0 value we can create a function called labeller that will take the
...
repli <- repli+
  labs(title = "Use of online platforms, apps varies — sometimes widely — by demographic group",
       subtitle = "\n% of U.S. adults in each demographic group who say they ever use...",
       caption = 'Note: White and Black adults include those who report being only one race and are not Hispanic. Hispanics are of any race. Not all numerical\ndifferences between groups shown are statistically significant (e.g., there are no statistically significant differences between the shares of\nWhite, Black or Hispanic Americans who say the use Facebook). Respondents who did not give an answer are not shown.\nSource: Survey of U.S. adults conducted Jan. 25-Feb. 8, 2021.\n\"Social Media Use in 2021\"\n',
...
reasons, I then selected similar Google fonts.
```{r include= FALSE}
library(extrafont)
# Problem with importing fonts. Error: "No FontName. Skipping". Solved using the following code:
library(remotes)
...
Download the .ttf file of the correspondent font type, load import it from the
...
```{r}
#FONTS
# Import and load the fonts from the selected folder
font_import("./fonts") # Import form .ttf file path
...
# Y axis labels (monospaced Google font for left justification between facets)
...
font_add("Libre Franklin",
         regular = "./fonts/LibreFranklin-Regular.ttf", # caption and X labels
...
-**Theme**
Add the setting of the theme relative to:
```{r}
repli <- repli+
  theme(
...
Also, set the theme settings for the background and general aesthetics:
```{r}
repli <- repli+
  theme(
...
- **Applying theme with Google fonts**:
![Fonts different from Google Fonts](./repli_fonts.png)

### Annotation

Annotation of the values inside the matrix is applied using `Geom_text()`. 
When the values are above 40(%) the font is in white and in bold, when they are
below, they are in black and in plain text. Color selection is created with an
individual scale. Bold/plain font is applied by an `ifelse` condition.
...
### Exporting the plot
To export the plot in png we can use the `{ragg}` library. Use the width and
height from the original plot (1260x1280 px)
```{r include= FALSE}
library(ragg)
```
...
```{r}
#EXPORT PLOT 
pngfile <- fs::path(knitr::fig_path(),  "rep_fonts.png")
agg_png(pngfile,
        width = 1260,
        height = 1280,
        units = "px",
...
![Fonts different from Google Fonts](./rep_fonts.png)
With the original plot:

![Original plot](./original_plot.png)

## Replication PARALLEL COORDINATES
...
better the . Also, the labels for the demographic main groups (age, gender,
income...) could be added. Optionally, the logos of the platform could be
displayed above their names since we tend to recognise even faster the social
media apps logos than their names.
...
solve that, using parallel coordinates may be helpful.
...
dataframe for our parallel coordinate plot's structure: we order according to 
...
dataframe and we create a column for "group" (as before). Then, we pivot wider
...
dataK <- dataK %>% 
  ...
  filter(key %in% c("Total", "Men", "Women", "White", "Black", "Hispanic",
                    ...
                    "$30K-49.9K", "$50K-74.9K", "$75K+", "HS or less", "Some college",
                    "College+", "Urban", "Suburban", "Rural")) |>
...
The idea to do this is creating individual parallel coordinate plots for each
demographic group, using an individual color scale for each one (color-hue 
scales). We will use `ggparcoord()` for this purpose.

We will need to discard the percentage values (geom_text()) since they
will be barely read.


We will repeat almost the same process: filter dataframe by the group, building
the ggplot (x axis the platforms, y axis the percentages) and seeting the line
size. In some cases we will need a reference vector to order the variables in
the legend.
...
### Fonts
Almost the same as in the replication plot:
```{r}
#Title font (main plot)
font_add("Libre Franklin Black",
         regular = "./fonts/LibreFranklin-Black.ttf",
         bold = "./fonts/LibreFranklin-Black.ttf")

# Subtitle font (main plot)
font_add("Libre Franklin Medium",
         regular = "./fonts/LibreFranklin-MediumItalic.ttf",
         italic = "./fonts/LibreFranklin-MediumItalic.ttf")

# Y and X axis labels and Caption text
font_add("Libre Franklin",
         regular = "./fonts/LibreFranklin-Regular.ttf", # caption and X labels
         bold = "./fonts/LibreFranklin-SemiBold.ttf")
...
I tried to keep the horizontal lines as subtle as possible (in order to help
the reader interpret the percentages) while giving emphasis to the vertical 
lines.
...
some elements from the theme of the replication.
```{r include=FALSE}
library(patchwork)
```
...
global <-gender+race+age+income+
  ...
  plot_layout(ncol = 3)+
  plot_annotation(title = "Use of online platforms, apps varies — sometimes widely — by demographic group",
                  subtitle = "% of U.S. adults in each demographic group who say the ever use...",
                  caption = 'Note: White and Black adults include those who report being only one race and are not Hispanic. Hispanics are of any race. Not all numerical differences between groups\nshown are statistically significant (e.g., there are no statistically significant differences between the shares of White, Black or Hispanic Americans who say the use\nFacebook). Respondents who did not give an answer are not shown.\nSource: Survey of U.S. adults conducted Jan. 25-Feb. 8, 2021. \n\"Social Media Use in 2021\"\n\nPEW RESEARCH CENTER',
...
### Exporting the plot
```{r}
pngfile <- fs::path(knitr::fig_path(),  "alternative_plot.png")
agg_png(pngfile,
        width = 1560,
        height = 1300,
        units = "px",
...
### Result
This is the final alternative version of the original plot:

![Alternative plot]("./alternative_plot.png")
...
project. Comments in .R are ok (if you don't forget to add them), but adding
...
iteratively. And don't forget to render them before trying the next one.
