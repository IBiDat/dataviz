...
```{r, eval=FALSE}
P_gamelog_reg <- suppressWarnings(game_logs(
  seasons = selectedSeasons, league = "NBA", result_types = "player",
...
```{r, eval=FALSE}
write.table(ff, file = "ff.txt", sep = "\t", row.names = F)
```
...
ff <- read.delim("ff.txt", header=TRUE)
```
