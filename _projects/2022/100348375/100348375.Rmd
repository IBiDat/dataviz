---
title: "Replication and alternative visualization of a New York Times market share 
graph"
description: |
...
```{r echo=FALSE, fig.cap="Source: New York Times", out.width = '100%'}
knitr::include_graphics("market_share.png")
```
...
graph first and then the lower one.
...
```{r fig.height=7, fig.width=6, fig.showtext= TRUE}
...
```{r fig.height=7, fig.width=6, fig.showtext= TRUE, warning=FALSE}
...
Lastly, I've also added a thicker vertical line marking 50, using the 
"geom_vline" function.
...
```{r fig.height=7, fig.width=6, fig.showtext= TRUE, results=FALSE, warning=FALSE}
...
upper_plot <- data %>%
  ...
  theme(legend.position = "none",
        plot.title = element_text(size = 12, 
                                  ...
                                  family = "Noto Serif", 
                                  margin = margin(0, 0, 5, 0)),
        plot.subtitle = element_text(size = 9.5, colour = "#636363", 
                                     family = "Noto Serif", 
                                     margin = margin(0, 0, 20, 0)),
        plot.title.position = "plot",
        plot.tag.position = c(0.12, 0.878),
...
```{r fig.height=2, fig.width=6, fig.showtext = TRUE}
...
bottom_plot <- data %>%
  ...
  labs(subtitle = "Technology companies since 2012",
       caption = 'By The New York Times | Source: IbisWorld and Open Markets Institute; "Early 2000s" ranges\n from 2002 to 2007, depending on data availability') +
  theme(legend.position = "none",
        plot.title.position = "plot",
...
```{r fig.height=9, fig.width=6.3, fig.showtext = TRUE}
...
The replication of the plot, created with the R package ggplot2, is very close to 
the original plot. There is a detail that I have not been able to replicate 
...
Finally I opted for the option of making a graph of parallel coordinates. 
...
than position on a common scale (as in the original graphic). However, I think 
it's also a good way to represent this data.
...
```{r fig.showtext=TRUE, fig.height=7, fig.width=6}
...
alternative_plot <- alternative_plot +
  labs(title = "Dominance of Corporate Behemoths",
       subtitle ="The combined market share of the two largest companies in many industries\n has grown in recent years, often more than 40%.",
       tag = c("Early 2000s", "Now"),
       caption = 'By Isabel Molero López | Source: New York Times; "Early 2000s" ranges from 2002 to 2007.\nLines that begin with a yellow dot correspond to the data of technology companies since 2012 ') +
  theme(legend.position = "none",
        ...
        plot.caption = element_text(size = 9.5, colour = "#636363", 
                                    family = "catamaran", margin = margin(15, 0, 0, 0),
                                    hjust = 0),
        plot.caption.position = "plot",
        panel.grid.major.y = element_line(linetype = "dotted", 
                                          colour = "#a7a7a7"),
        ...
        axis.text.y = element_text(size = 10, family = "catamaran", 
                                   hjust = 0.7, vjust = 0.5, face = c('plain', 'plain', 'plain', 
                                            'bold', 'plain','plain', 
                                            'plain', 'plain', 'plain')),
        axis.title.x = element_text(hjust = 0.78, margin = margin(0, 0, 20, 0), size = 11.5, family = "catamaran", colour = "#636363",
                                   face = "bold"),
        axis.title.y = element_text(size = 11, family = "catamaran", colour = "#636363", margin = margin(r = 7)),
...
alternative_plot <- alternative_plot +
  ...
  geom_point_interactive(data = data.frame.points, 
                         aes(x, market_share_2000,
                             ...
                             data_id = 1),
             colour = "#E6B400") +
  annotate_interactive("text", x = 1.09, y = 84.7, label = "2012", size = 3.4, color = "#E6B400",
           fontface = "bold", family = "catamaran",
           tooltip = "", data_id = 1) +
  annotate_interactive("text", x = 1.11, y = 88, label = "Technology", size = 3.1, color = "#E6B400",
           fontface = "bold", family = "catamaran",
           tooltip = "", data_id = 1) +
  annotate_interactive("text", x = 1.11, y = 86.5, label = "companies", size = 3.1, color = "#E6B400",
           fontface = "bold", family = "catamaran",
           tooltip = "", data_id = 1)
...
Here I show the complete code used for the elaboration of the static graph:
...
alternative_plot <- alternative_plot_data %>% 
  ...
  geom_segment_interactive(aes(x = `Early 2000s`, xend = Now, 
                               ...
                               col = increase2,
                               alpha = increase2,
                               tooltip = paste0(industry, " ", perc_increase_label, "%"),
                               data_id = industry), 
               size = 0.71, show.legend = F) +
  geom_vline(xintercept = 1, size = 0.5, color = "gray") + 
  geom_vline(xintercept = 2, size = 0.5, color = "gray") +
  ...
  labs(title = "Dominance of Corporate Behemoths",
       ...
       caption = 'By Isabel Molero López | Source: New York Times; "Early 2000s" ranges from 2002 to 2007.\nLines that begin with a yellow dot correspond to the data of technology companies since 2012 ') +
  theme(legend.position = "none",
        plot.title = element_text(size = 14, face = "bold", family = "catamaran", 
                                  margin = margin(0, 0, 5, 0)),
        plot.subtitle = element_text(size = 12, colour = "#636363", 
                                     family = "catamaran", 
                                     margin = margin(0, 0, 30, 0)),
        plot.title.position = "plot",
        plot.tag.position = c(0.11, 0.88),
