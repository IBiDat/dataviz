...
description: |
  A layer-by-layer construction of a map of China containing information on forecasted GDP growth and population for several cities.
categories: "2022"
...
In order to obtain data on the "longitude" and "latitude" of each city, we used the [`world.cities` database](https://search.r-project.org/CRAN/refmans/maps/html/world.cities.html). This database includes data on these and other variables for over 40,000 cities in the world. I then filtered the cities in China and selected (as well as renamed) the variables of interest to later on construct my final database.
...
The final step in data processing was obtaining the data to plot the underlying map of China. We used the [`map_data` function](https://ggplot2.tidyverse.org/reference/map_data.html) and again, filtered, selected and renamed variables accordingly.
...
We then use `geom_path` to set the line demarcating the limits of the different provinces. We then set its `color` and `size`. Further, `coord_map()` sets the coordinate system for the map by projecting the defined portion of the Earth onto a flat 2D plane. 

```{r, out.width="100%"}
p <- ggplot(data=china,
            aes(x=Longitude, y=Latitude, group=group)) +
  geom_polygon(fill="cornsilk3", alpha=0.3) +
  geom_path(color="white", size=0.2) +
  coord_map() 
p
```

Then we add the data points at the specific locations of the 104 cities (defined by their latitude and longitude) with `geom_point()`. Thus, on top of the map, we are plotting a scatterplot where each point represents one city. The `size` of the point (more specifically, its area) represents its population and its `fill` depicts the relative position of a given city vis-à-vis other cities in the EIU ranking. We then set a black line around the point (`shape=21`) while specifying the `color` and thickness (`stroke`) of the border. 
