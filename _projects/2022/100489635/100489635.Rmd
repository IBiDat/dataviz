...
p4 <- p3 + 
  ...
  annotate("text", x=58, y=37, label= "Highest ranked", size=6, hjust=0, vjust=0, angle=0,
           fontface = "bold", family = "IBM Plex Sans", color="blue4") +
  annotate("text", x=58, y=15, size=6, hjust=0, vjust=0, label=  
  "1  Hangzhou
2  Shenzhen
3  Shanghai
4  Suzhou
5  Guangzhou", family="IBM Plex Sans") +
  ...
  annotate("text", x=115, y=21, size=6, hjust=0, vjust=0, 
  ...
  annotate("text", x=103, y=23, size=6, hjust=0, vjust=0, 
           label="Guangzhou", family="IBM Plex Sans") +
  annotate("text", x=116, y=41, size=6, hjust=0, vjust=0, 
...
Moreover, despite being a small detail, the choice of the color (hue) palette may not be the most appropriate to convey the order of cities according to their EIU ranking. There does not seem to be an obvious reason for why stronger colors are assigned to the extremes in the ranking spectrum (i.e. highest- and lowest-ranked cities). Hence, allocating different levels of saturation or lightness of the same hue could be an alternative way of conveying the 'ordering' of cities according to ranking. In the code above, we simply change the last three colors in the 'scales' section.
...
Finally, in order to emphasize the idea that cities with the highest growth potential are on the coast, we might want to insert a vertical line (I use `geom_vline()`) at the (arbitrary) level of 112°. This way we can see that among the top 10 cities of highest growth potential, only Chengdu is west of this longitude.
...
The final section of this post proposes an alternative visualization to represent the relationship between the population, location and growth potential of cities in China. One possibility is the scatterplot shown below. We plot the ranking on the x-axis and population on the y-axis (this time changing the default scientific notation by merely using 'M' to represent 'million'), through the `geom_point()`function. We keep the title and subtitle from the original graph, as well the main theme elements.
...
However, we still need to include information about the coordinates of cities. Within the location variable, it seems that longitude is the most important factor: cities closer to the coast (i.e. farther east, or higher values of x) appear to have higher growth potential. Therefore, we could code longitude through the color of the points by highlighting in blue points representing cities that are located east of the (arbitrarily chosen) 112° meridian. I also add a text label (on top of a white rectangle constructed with `geom_rect()`) to indicate blue dots represent the cities farther east in China.

```{r}
annotation <- dplyr::filter(fulloutputclean, Longitude>112)
p <- p + 
    geom_point(data=annotation, color="#6495ED") +
    geom_rect(aes(xmin=79, xmax=127, ymin=14300000, ymax=15900000), fill="white", color = "#6495ED") +
    geom_text(aes(x=80, y=15000000, label="Cities to the east of the meridian 112°"), size=6,
              hjust=0, vjust=0, color = "#6495ED")
p
```

Finally, to highlight the top 5 cities from the original map, I construct another annotation and insert it as an argument in the `geom_text()` function. We can see that highly-ranked cities (i.e. cities with a low score) are all near the coast (i.e. east of the 112° meridian), but have rather different population levels.
