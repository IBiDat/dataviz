...
These are two charts created by the team at El Orden Mundial. As can be seen, they are both very interesting and distinct representations, showcasing the great visualization work of this platform. On one side, we have a map displaying the main cocaine import routes in Europe. On the other, there is a stacked area chart showing the ideological composition of the U.S. Supreme Court. These charts, in addition to being clear and relatively easy to read, use appropriate chart types for each issue and also feature a well-thought-out color scheme.

![Rutas de Cocaína hacia Europa](https://elordenmundial.com/wp-content/webp-express/webp-images/doc-root/wp-content/uploads/2024/02/mapa-trafico-cocaina-europa-ordenador-def.png.webp)


![Corte Suprema EEUU](https://elordenmundial.com/wp-content/webp-express/webp-images/doc-root/wp-content/uploads/2024/11/corte-suprema-eeuu-ordenador.png.webp)

The next one, is the original plot that I want to reproduce:
![Original Plot](https://elordenmundial.com/wp-content/webp-express/webp-images/doc-root/wp-content/uploads/2019/03/Mundo-Econom%C3%ADa-Sociedad-Salud-Droga-Consumo-de-coca%C3%ADna-1310x926.png.webp)

This is my final replica which I am going to create in the following steps:
```{r preview, echo=FALSE, out.width="100%"}
knitr::include_graphics("plot_logo.png")
```
...
# I needed to convert the archives
tryCatch({
    image <- image_read("Scotland.png")
    image_write(image, "Scotland_fixed.png", format = "png")
    print("Archivo convertido exitosamente a PNG válido: Scotland_fixed.png")
}, error = function(e) {
    print(paste("No se pudo convertir el archivo: ", e$message))
})
tryCatch({
    image <- image_read("England and Wales.png")
    image_write(image, "England and Wales_fixed.png", format = "png")
    print("Archivo convertido exitosamente a PNG válido: Scotland_fixed.png")
}, error = function(e) {
    print(paste("No se pudo convertir el archivo: ", e$message))
})
tryCatch({
    image <- image_read("Northern Ireland.png")
    image_write(image, "Northern Ireland.png", format = "png")
    print("Archivo convertido exitosamente a PNG válido: Scotland_fixed.png")
}, error = function(e) {
    print(paste("No se pudo convertir el archivo: ", e$message))
})
...
Escocia <- png::readPNG("Scotland_fixed.png")
Inglaterra <- png::readPNG("England and Wales_fixed.png")
España <-  readPNG(flagon::flags("es"))
Paises_Bajos <- readPNG(flagon::flags("nl"))
Irlanda_norte <- png::readPNG("Northern Ireland.png")
...
#Save this graph as an image to show in the introduction 
ggsave("plot_logo.png", plot_logo, width = 8.5, height = 6)
```

## CRITICS TO THE ORIGINAL GRAPH:
...
upgrade <- ggplot(cocaine_data, aes(x = reorder(País, `Porcentaje de consumo (%)`),  # Reorder countries by consumption percentage
                                    y = `Porcentaje de consumo (%)`,
                                    fill = `Porcentaje de consumo (%)`), 
                  family = "mulish", font.main = 2) +
...
cocaine_data$Continente <- c("Oceanía","Europa","América","Europa","Europa","Europa","Europa","Europa","América","América","Europa","América","Europa","América","América")
