...
Press freedom is "the ability of journalists as individuals and collectives to select, produce, and disseminate news in the public interest independent of political, economic, legal, and social interference and in the absence of threats to their physical and mental safety."

![Original chart. Source:<https://rsf.org/es/clasificaci%C3%B3n-mundial-de-la-libertad-de-prensa-2024-el-periodismo-bajo-las-presiones-pol%C3%ADticas>](originalplot.jpeg){.external width="100%"}

As we can see, the graph we are going to analyze is a stacked column chart that portrays the press freedom situation in each continent this year. For scores lower than 40, press freedom is considered a very serious situation; between 40 and 55, difficult; between 56 and 70, problematic; between 70 and 85, satisfactory; and from 85 to 100, good.
...
First, in order to create our graph, we load the data:
...
The original graph is completely fine and does its job at showing the different levels of press freedom in 2024. But the RSF study contains so much more information that could be displayed with just a little bit more interactivity that it is worth generating other graphs.
...
To have a more global perspective on press freedom across all the years of the study, we create a Shiny App with two tabs, one being an animated bar chart and another being a static stacked area chart. At the left of the page, we have the option to choose the zone we want to look at.

In the animated bar chart, by clicking the play button at the bottom right part of the graph, we can travel across the years and see how the bars move as the press freedom situation changes. By doing this -having added the rest of the years of the study into the dataset- we are allowing users to select the year they are interested in and not only the last two.

The stacked area chart works for those interested in evaluating not a year in particular but the change in slopes across the years of the study. Here, we can better observe the trends and the up and downs of the different categories. This is also something that cannot be seen with just the original graph.
...
zone_levels      <- levels(percentage_data_all$Zone)
situation_levels <- levels(percentage_data_all$Situation)
year_values      <- sort(unique(percentage_data_all$Year))
...
# Finally call shinyApp() to launch it:
shinyApp(ui = ui, server = server)

```

The insights we can get from all these graphs is worrying: governments are failing to guarantee an exemplary framework for the practice of journalism and the right of citizens to reliable, independent and pluralistic information. 
