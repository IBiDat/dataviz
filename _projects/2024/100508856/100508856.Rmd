...
[here]
1.(https://ourworldindata.org/grapher/annual-working-hours-per-worker?time=1870..2017)
2.(https://ourworldindata.org/grapher/annual-working-hours-per-worker?tab=map&time=2005)
...
library(ggokabeito)
library(paletteer)
working_hours<-read.csv("C:/Users/liaom/Desktop/Courses/数据可视化/pre/123/annual working hours/annual-working-hours-per-worker.csv")
...
data <- read.csv("C:/Users/liaom/Desktop/Courses/数据可视化/pre/123/annual working hours/annual-working-hours-per-worker.csv")
...
tm_shape(world_1900) +
  ...
  tm_layout(
    ...
    legend.width=0.6,
    legend.height=0.7,
    legend.title.color="#8B1C62",
    legend.title.size=0.7,
    legend.text.color="#008B8B",
...
world_97 <- left_join(World,data_97,by="sovereignt")
tm_shape(world_97) +
  tm_polygons("Hours") +
  tm_layout(
...
The original graph is the evolutionary path of the data of different countries superimposed, and then dynamic simulation is carried out. In this part, I select several regions to draw one of them. Here is a link to the original graph:(https://ourworldindata.org/grapher/annual-working-hours-vs-gdp-per-capita-pwt?time=earliest..2019&country=BRA~USA~DEU~TWN%C2%AEion~PRT~ESP~AUS~NZL~ZAF)


```{r}

data <- read.csv( "C:/Users/liaom/Desktop/Courses/数据可视化/pre/123/GDP.csv")
...
```{r}
data <- read.csv("C:/Users/liaom/Desktop/Courses/数据可视化/pre/123/annual-working-hours-vs-gdp-per-capita-pwt/annual-working-hours-vs-gdp-per-capita-pwt.csv")
colnames(data)[c(4,5)] <- c("work_time","GDP")
data1 <- data[-which(is.na(data$GDP)),]
data2 <- data1[-which(is.na(data1$work_time)),]
data3 <- dplyr::filter(data2, grepl('Brazil|China|France|Germany|Hong Kong|India|
