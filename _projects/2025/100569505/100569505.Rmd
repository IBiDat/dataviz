...
How do we decide how to move around a city? For many commuters using sustainable transport modes, the choice between cycling and public transit is highly influenced by a decisive factor: travel time.

Our project analyzes and replicates a data visualization created by Willem Klumpenhouwer, which compares travel times by bike and transit in the city of Calgary, the largest city in Alberta, Canada. The map shows the difference in the time it takes to get to Calgary Tower by bike and by transit from any point of the city.

Beyond assisting individual commuters, this visualization serves as a diagnostic tool for policy makers. By mapping the relative competitiveness of sustainable transport modes across the entire city, it highlights spatial inequities in transit access and identifies neighborhoods where investments in infrastructure could more needed or effective.
...
library(smoothr) # To smoothen out the hexagonal borders of the isochrones
library(ggnewscale) # To reset scale colors
library(scales) # To define alpha in arguments where defaults dont't allow it
...
Initially, we developed the replica using the exact dimensions of the original map (25.2565 x 34.374 inches using RMD's default of 96 dpi) in order to be able to replicate the sharpness in its elements. However, after receiving feedback informing about storage constraints, we changed those values to comply with a significantly smaller file size. This meant that we needed to adjust many values (like linewidth, text size, etc.). 

Instead of manually recalculating everything, we introduced a scaling ratio (`sr`). This object captures the proportion between our new target height (13.5 inches) and the original height. By multiplying our aesthetic values by `sr`, we can dynamically scale the entire visualization while preserving the relative proportions of the original design. This method also allows for easier changes if we want to perform any size changes in the future.
...
To resolve this, we need to dissolve the internal boundaries between adjacent hexagons of the same class. Using group_by followed by summarise is an elegantly simple solution to merge these features into single geometries.
...
I used this code to find an adequate area of streets to use as arrow:
...
# Blue point:
legend_blue_point_x <- -114.1880 #-6
...
border_thickness <- (0.1 * sr) # Range 0-1 (no border, full black box).
...
# SAVE
ggsave(
  filename = "/home/odradek/css_master/1c_data_visualization_inaki_ucar_marques/adjusting_smaller/adjalt.png",
  width = 10.03909,    
  height = 13.5,       
  dpi = 96,          
  units = "in",       
