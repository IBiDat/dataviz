...
## Adding "isochrones" for absolute travel time
...
hexagons2 <- hexagons |> 
  mutate(
    "fastest_travel_time" = case_when(
      ...
      b_travel_time <= travel_time ~ b_travel_time
    )
  )

# Plotting:
ggplot() + 
  geom_sf(data = hexagons2,
          aes(fill = fastest_travel_time,
              color = fastest_travel_time)
          ) + 
  scale_fill_gradient(low = "black", high = "white") +
  scale_color_gradient(low = "black", high = "white") +
  new_scale_color() + # Reset the scale for the next geoms

# THEN PLOT EVERYTHING ELSE ON TOP (JUST ROADS FOR THE MOMENT)
  geom_sf( # We plot the roads:
    data = hexroads_intersection,
    aes(color = time_color),
    linewidth = 1.1,  # To make roads thicker
...
hexagons2 <- hexagons |> 
  mutate(
    "fastest_travel_time" = case_when(
      ...
      b_travel_time <= travel_time ~ b_travel_time),
    "fastest_travel_time" = case_when(
      fastest_travel_time < 15 ~ 1,
      fastest_travel_time < 30 ~ 2,
      fastest_travel_time < 45 ~ 3,
      fastest_travel_time < 60 ~ 5,
      fastest_travel_time < 75 ~ 6,
      fastest_travel_time < 90 ~ 7,
      fastest_travel_time < 105 ~ 8,
      fastest_travel_time < 120 ~ 9,
      fastest_travel_time >= 120 ~ 10))
```


## Adding "isochrones" for time by bike
...
ggplot() + 
  geom_sf(data = hexagons2,
          aes(fill = b_travel_time,
              color = b_travel_time)
          ) + 
  scale_fill_gradient(low = "#c88c47", high = "white", na.value = "red") +
  scale_color_gradient(low = "#c88c47", high = "white", na.value = "red") +
  new_scale_color() + # Reset the scale for the next geoms
...
# THEN PLOT EVERYTHING ELSE ON TOP (JUST ROADS FOR THE MOMENT)
  ...
  scale_color_manual(
    values = c(
      "-2" = "#000000",   # bike >30 min faster
      "-1" = "#000000",   # bike 10–30 min faster
      "0"  = "#b7a6ac",   # similar travel time
      "1"  = "#a36f96",   # transit 10–30 min faster
      "2"  = "#6a4886"    # transit >30 min faster
    )
...
```{r}
# Creating discrete travel time by bike:
hexagons2 <- hexagons |> 
...
# THEN PLOT EVERYTHING ELSE ON TOP
  geom_sf( # We plot the roads:
    data = hexroads_intersection,
    aes(color = time_color),
    linewidth = 1.3,  # To make roads thicker
    show.legend = FALSE
  ) +
  scale_color_manual(
    values = c(
...
```
```{r}
# 1. PRE-PROCESS: Merge hexagons into smooth "zones"
# We group by the time value and dissolve internal boundaries
hex_zones <- hexagons2 |>
  group_by(iso_b_travel_time) |>
  summarise(geometry = st_union(geometry)) |>
  ungroup()

# 2. PLOT
ggplot() + 
  # Plot the ZONES, not the original hexagons
  geom_sf(
    data = hex_zones,
    aes(fill = iso_b_travel_time, color = iso_b_travel_time),
    alpha = 0.7
  ) + 
  scale_fill_gradient(low = "#c88c47", high = "white", na.value = "white") +
  scale_color_gradient(low = "#c88c47", high = "white", na.value = "white") +
  new_scale_color() + # Reset scale color
...
## New idea: bike in roads, transit difference in map
...
- Mergin polygons idea: https://stackoverflow.com/questions/49354393/r-how-do-i-merge-polygon-features-in-a-shapefile-with-many-polygons-reproducib
