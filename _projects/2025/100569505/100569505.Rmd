...
# Horizontal line:
line_length <- 1 # Proportion of the box width
line_space <- 1 # Space between line and box, proportion of box width
line_color <- "#676767"
line_thickness <- 1 # For argument linewidth =

# Ticks:
tick_length <- 1 # Proportion of the box height
tick_color <- "#000000"
tick_thickness <- 1 # For argument linewidth =

# Kilometers:
km_space <- 1.66 # Space between km and line, proportion of box height
km_size <- 9 # In points, not relativ to the map
km_color <- "#000000"
km_font <- "atkinson"

scale_km <- tibble( ############### NEED TO FINISH THIS ONE, BUT BEFORE THAT, GET THE SCALE RIGHT###############
  label = c("0", "1.5", "3", " km")
  x = c(
...
# Then the tibble:
horizontal_line <- tibble(
  x = box_mid_x - ((box_actual_width/2)*line_length),
  xend = box_mid_x + ((box_actual_width/2)*line_length),
  y = arrow_box$ymin - (box_actual_height*line_space),
  yend = arrow_box$ymin - (box_actual_height*line_space))


### Ticks:
tick <- tibble(
  x = c(horizontal_line$x, box_mid_x, horizontal_line$xend),
  xend = c(horizontal_line$x, box_mid_x, horizontal_line$xend),
  y = horizontal_line$y - ((box_actual_height/2)*tick_length),
  yend = horizontal_line$y + ((box_actual_height/2)*tick_length))
...
```
**I've been think the wrong way about how to plot the line for the scale, i was trying to just imitate the position comparing the pictures, but i need to calculate what a km (and 3km) are in my map in order to properly plot the scale!** 
...
ggspatial:::GeomScaleBar$draw_panel
