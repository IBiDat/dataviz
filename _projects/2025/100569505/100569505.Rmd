...
hexagons2 <- hexagons |> 
  mutate(
    ...
    "fastest_travel_time" = case_when(
      ...
      fastest_travel_time >= 60 ~ 4)) |> 
  filter(fastest_travel_time != 4) # So that we don't get an area for the 
...
# To fix holes (se more detail below)
hex_zones <- hex_zones |>
  ...
  slice_max(area, n = 1) |>  # We keep only the polygons with the bigger ares
...
# A) We first plot the isochrone's areas (no borders yet)
  ...
  scale_fill_gradient(low = "green", # Central isochrone
                      high = "red" # Outer isochrone
                      ) +
  new_scale_color() + 
...
# ============================
# When plotting hex_zones small holes appear. I need to get those removed.
