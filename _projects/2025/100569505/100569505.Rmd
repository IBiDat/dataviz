...
Initially, we developed the replica using the exact dimensions of the original map (25.2565 x 34.374 inches using RMD's default of 96 dpi) in order to be able to replicate the sharpness in its elements. However, after receiving feedback informing about storage constraints, we changed those values to comply with a significantly smaller file size. This meant that we needed to adjust many values (like line width, text size, etc.). 
