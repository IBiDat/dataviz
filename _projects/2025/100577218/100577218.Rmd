...
```{r BASE MAP - COLOURS FOR ARC GRAPHS}
color_points <- tibble(
...
```{r BASE MAP - DATA CREATION FOR COUNTRY GRAPHS}
...
```{r LINES FROM ARC PLOTS AND BOXES}
...
```{r KEY - ADD ALL PARTS TO THE WHITE BOX}
# Plot flags
...
```{r PLOT FINAL MAP, fig.width=fig.width, fig.height=fig.height}
base_map
```
...
```{r IMPROVEMENT GRAPH 1, fig.width=8, fig.height=9}
...
```{r IMPROVEMENT GRAPH FINAL fig.width=7, fig.height=7}
bar_data_3 <- filtered_data |>
  pivot_longer(
    cols = c(human_impact, incidence, geopolitical_impact, total_fatalities),
    names_to = "metric",
