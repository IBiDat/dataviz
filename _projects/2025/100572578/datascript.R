...
fair <- effort_share |> 
  pivot_longer(!`Upper end of`, 
               ...
               values_to = "emissions") |> 
  filter(year == 2030, 
         `Upper end of` == "1.5°C Paris Agreement compatible") |> 
  mutate (year = as.numeric(year))

mod_1.5 <- country |> 
  select(`1.5°C compatible`, year) |> 
  filter(year == 2030) |> 
  pivot_longer(`1.5°C compatible`,
               names_to = "modelled",  
               values_to = "emissions")
...
dom_data <- country_1 |>  
  filter(indicator == "Modelled Domestic Pathways boundaries", 
         year <= 2030, 
         year >= 2024,
         scenario == "1.5°C compatible") |> 
  select(year, emissions, scenario) |> 
  add_row(
    year = c(2023),
    emissions = c(454.97),
    scenario = "1.5°C compatible - ref line", 
    .before = 1)
...
fair_data <- country_1 |>  
  ...
  add_row(
    ...
    scenario = "1.5°C compatible - missing data", 
    .before = 1)
