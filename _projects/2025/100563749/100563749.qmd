...
font_add("TitleFont", "Montserrat-Bold.ttf")
font_add("SubtitleFont", "Montserrat-Regular.ttf")
...
map_aug2025 <- ggplot() + 
  ...
  geom_sf(data = map_data_aug2025,
          mapping = aes(fill = bi_class), # Bi-class crafted earlier
          colour = "white",               # County lines 
          size = 0.0005,                  # Width of county lines 
          show.legend = FALSE) +
  # State lines
  geom_sf(data = states,
          fill = NA,        # States are transparent
          colour = "white", # State lines
          size = 0.2,
          show.legend = FALSE) + 
  # Colour scale 
...
```{r feb23aug25-compariso, fig.width = 14, fig.height = 10}
...
improved_map_data <- improved_map_data |> 
  mutate(
    ...
    travel_cost = total_gas_cost / 
      (Median_Household_Income_2022 / 12)  # Monthly income
    ) 
```
...
```{r TC-comparison, fig.width = 14, fig.height = 10}
# Map titles
...
# Updating distance title for explanatory legend
...
final_distance_vs_cost_plot <- plot_grid(
  ...
  legend,                                # Main bivariate legend
  maps_comparison_distance_cost,         # Maps side by side
  ncol = 1,
  rel_heights = c(
    0.28,  # How-to-read block
    0.12,  # Main legend
...
```{r bubble-map, fig.width = 14, fig.height = 10}
