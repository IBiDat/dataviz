...
improved_civilwar_plot <- ggplot(
  duration_data,
  aes(x = start_year, y = duration, size = deaths_thousands, color = deaths_thousands)
) +
...
Here is another suggestion for improvement. I think this can look good when the deadliest conflicts get labels as well as the longest ones, can also label the peaks for the total deaths.

```{r second_improvement, fig.width=11, fig.height= 10}

 duration_data <- plot_data |> 
  mutate(
    duration = end_year - start_year,  # Calculate duration
  ) |> 
  group_by(region) |> 
  mutate(
    is_deadliest = deaths_thousands == max(deaths_thousands),  # Deadliest in region
    is_longest = duration == max(duration),  # Longest in region
    # Create label
...
geom_line(data = deathtotal,
 ...
 scale_color_gradient(
  ...
  scale_y_log10(
    labels = scales::comma,
    breaks = c(1, 10, 100, 400, 600)
  ) +
  scale_x_continuous(
    breaks = seq(1946,2012,10)
    
  ) +
  coord_cartesian(xlim = c(1944, 2014), ylim = c(-4, 650), expand = FALSE) +
  ...
  theme(
    ...
    plot.title = element_text(size = 16, face = "bold", color = "gray20", margin = margin(b = 5)),
    ...
    plot.margin = margin(t = 40, r = 15, b = 10, l = 10)
  )
