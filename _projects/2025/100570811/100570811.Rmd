...
The original graph consists of two plots. One plot on the top showing total deaths for all civil wars per year from 1946-2012, with year on the x-axis and death toll on the y-axis. The other plot visualizes the duration and deadliness of the top 100 deadliest civil wars from 1946-2012. the y-axis represents the death toll rank, while the x-axis is the exact same as in the top plot with years. The lines represent each civil war and the lenght of them shows the duration of the conflict. Moreover, the colors represent the region and the numbers on the labels beside the state name is deaths in thousands for that exact conflict.

The graph is trying to convey the deadliness and duration of civil wars from post-war times to modern times. As one can see the overall deadliness has decreased, but that the civil wars are lasting longer, like in Afghanistan and Sudan to mention some.

The main reason for deciding on this graph is because it visualizes data in an informative way, thus supporting the Economist article on the actual deadliest conflicts in the world. The problem with the graph is that it overdone, resulting in too much information which makes interpretation hard. The motivation was therefore to replicate it and then improve it to show that a simplification and reduction of the amount of data visualized, still can be as informative and even more comprehensible.
...
totaldeaths <- read.csv("deaths-in-state-based-conflicts.csv")

totaldeaths <- totaldeaths |> 
  filter(between(Year, 1946, 2012), Entity == "World")  


# Filter population data for 1946-2012 and calculate total world population per year
population <- read.csv("population-regions-with-projections.csv")

world_population <- population %>%
  filter(between(Year, 1946, 2012)) %>%
  group_by(Year) %>%
  summarise(
...
# Join and calculate deaths per 100,000
deaths_per_capita <- totaldeaths |> 
  left_join(world_population, by = "Year") |>
  mutate(deaths_per_100k = (Deaths.in.ongoing.conflicts..best.estimate....Conflict.type..state.based / Population) * 1000000)
```
...
# Create the dataset with all civil wars
civil_wars_data <- data.frame(
  ...
  deaths_thousands = c(
    ...
    # Europe
    77.0, 27.0, 17.6, 2.4, 11.6, 3.6, 8.9, 4.3, 3.9, 2.6, 2.5,
...
deathplot <- ggplot(deaths_per_capita) +
  aes(x = Year, y = deaths_per_100k) +
 scale_y_continuous(
    limits = c(0,250),
    breaks = seq(0, 250, 50)
  ) +
...
deathplot <- deathplot +
  ...
  theme(
    ...
    # Grid lines (only horizontal major lines)
...
```{r civilwar_annotation, fig.width=8, fig.height= 8}
 ...
 civilwar_plot <- civilwar_plot +
   geom_rect(data = grey_strips,
            ...
            alpha = 0.5,
            inherit.aes = FALSE) +
  annotate("rect", xmin=1942, xmax=1944, ymin=0,ymax=100, alpha=0.4, fill="grey90") +
  annotate(
    "text",
    x = 1943,            
    y = 20,              
  ...
  annotate(
    "text",
    x = 1943,            # left margin near the y-axis
    y = 90,              
    label = "Lowest",
    angle = 90,
  ...
  annotate(
    "text",
    x = 1943,            # left margin near the y-axis
    y = 3,              # midpoint between 1 and 100
    label = "Highest",
    angle = 90,
    hjust = 1,           # right-align text
    size = 3,
  ...
  annotate("rect", xmin=c(1946, 1955, 1965, 1975, 1985, 1995, 2005)
...
```{r civilwar_themes, fig.width=8, fig.height= 8}
 ...
 civilwar_plot <- civilwar_plot +
  ...
  theme(panel.grid.major = element_blank(), 
        ...
        plot.margin = margin(t = 5, r = 5, l = 5),
...
footer <- ggdraw() + 
  draw_label(left_caption, x = 0.01, hjust = 0, size = 8, color = "gray30") +
  draw_label(right_caption, x = 0.99, hjust = 1, size = 8, color = "gray30")
```

```{r legend}

# Create custom legend with 3 rows
