...
civil_wars_data <- read.csv("clean_data/civil_wars_data.csv")
...
deathplot <- ggplot(deathtotal) +
 ...
 scale_y_continuous(
  ...
  coord_cartesian(xlim=c(1942,2014), expand=FALSE) # Expanded limits to fit labels without cutting off data
...
civilwar_plot <- ggplot(plot_data, 
  ...
  coord_cartesian(
    ...
    ylim = c(0, 103),
    expand = FALSE,
    clip = "off" # Allows text annotations to extend outside the plot area
  ) +
...
```{r civilwar-data, fig.width=8, fig.height= 10}
 ...
 civilwar_plot <- civilwar_plot +
   # segments for completed conflicts
   geom_segment(aes(xend = end_year, yend = rank),
               linewidth = 1,
               data = filter(plot_data, end_year < 2012),
               show.legend = FALSE) + 
   # segments with arrows to indicate conflicts still ongoing in 2012
  geom_segment(aes(xend = end_year, yend = rank),
               linewidth = 1,
               data = filter(plot_data, end_year == 2012),
...
# Creating dummy plots to extract specific legend keys for the custom layout

# one row with only East Asia, South-East Asia & Oceania
...
# combining individual legend rows vertically to replicate the original 3-row layout
...
```{r data-improvement}
 ...
 duration_data <- plot_data |> 
  ...
  mutate(
    ...
    # Only label the conflict if it is EITHER the deadliest OR the longest in its region
