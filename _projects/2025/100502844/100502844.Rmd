...
![Original chart. Source: World Data Visualization Prize](images/clipboard-2056465238.png){.external}
...
```{r libraries-and-ptions, message=FALSE, warning=FALSE}
library(tidyverse)
library(janitor)
library(wbstats)
library(countrycode)
...
```{r fonts-and-colour,message=FALSE,warning=FALSE}
...
```{r helper-fetch-web, message=FALSE, warning=FALSE}
...
```{r economic-data, message=FALSE, warning=FALSE}
...
```{r governance-data, message=FALSE, warning=FALSE}
...
```{r hdi-data, message=FALSE, warning=FALSE}
...
```{r vdem-data, message=FALSE, warning=FALSE }
...
```{r size-groups, message=FALSE, warning=FALSE}
w <- d.econ17 |>
  transmute(
...
```{r  merge-everything, message=FALSE, warning=FALSE}
w1 <- w |>
...
```{r costom-theme, message=FALSE, warning=FALSE}
...
```{r text-blocks, message=FALSE, warning=FALSE}
...
```{r main-scatter-plot,fig.width=30, fig.height=20}
...
```{r ridge-plot, fig.width=14, fig.height=10}
...
```{r world-map-panel, fig.width=7, fig.height=6}
...
```{r bottom-text-panel, fig.width=10, fig.height=3}
...
```{r  assamble-figure, fig.width=39, fig.height=24}
...
```{r libraries-options, message=FALSE, warning=FALSE}
...
```{r visual-configuration , message=FALSE, warning=FALSE}
...
```{r data-integration-derived-varibles,message=FALSE, warning=FALSE}
...
```{r size-groups-snapshots-decoupling,message=FALSE, warning=FALSE}
...
```{r world-map, fig.width=9, fig.height=6,message=FALSE, warning=FALSE}
...
```{r panel-a-scatter, fig.width=40, fig.height=23, preview=TRUE,message=FALSE, warning=FALSE}
...
```{r helper-for-log1p-c02-axis-labels,message=FALSE, warning=FALSE}
...
```{r panel-b-democracy, fig.width=12, fig.height=6,message=FALSE, warning=FALSE}
...
```{r panel-c-co2, fig.width=12, fig.height=6,message=FALSE, warning=FALSE}
# Panel C – CO2 (log)
figC <- df_model |>
  dplyr::filter(
    !is.na(size_ridge),
...
```{r panel-d-hdi, fig.width=12, fig.height=6,message=FALSE, warning=FALSE}
# Panel D – HDI
figD <- df_model |>
  dplyr::filter(
    !is.na(size_ridge),
...
```{r panel-e-data, message=FALSE, warning=FALSE}
eff_data <- df_model |>
  dplyr::filter(
    !is.na(size_group),
    !is.na(hdi_per_co2_latest),
...
```{r panel-e-co2-absolute, fig.width=12, fig.height=6,message=FALSE, warning=FALSE}
...
```{r panel-e-hdi-per-co2, fig.width=12, fig.height=6,message=FALSE, warning=FALSE}
# Right efficiency panel – HDI per CO2
figE_right <- eff_data |>
...
```{r bottem-text-panel, fig.width=12, fig.height=3,message=FALSE, warning=FALSE}
...
#### Save panels and Assemble Layout

```{r assemble-layout, fig.width=49.1, fig.height=27, message=FALSE, warning=FALSE}
