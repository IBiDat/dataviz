...
World Economic Outlook (WEO). Having said the previous, the first chunk
exposes the loading of both databases.
...
The second and third chunk shows the procedure of cleaning of both
databases and the selection of countries.
...
The third chunk shows the cleaning and selection procedure for the R&D
database.
...
original databases, some work still needs to be done. The following
chunks show the creation of the variable eu_countries and the
group-region add in both databases.
...
data. The following chunk modifies p by plotting geom_area.
...
The following chunk changes the axis to adequate them to the WSJ
reference, taking into account that the y- axis is right-hand placed.
...
no information, such as year. Hence, the following chunk orders the
colors in blue and gold, and eliminate the labels of the axis.
...
within the graph, so there is no utility in keeping it. The following
chunk eliminates the group label, include the labels within the data,
puts the title in bold and includes some theme_minimal adjustments to
...
will leave this graph stored as p, and start building the R&D graph as
p1, to then merge them with patchwork.
...
colors rather than to repeat them twice. The following chunk replicates
this process, to be able to graph the R&D part.
...
centered. The following chunk does this and builds west_data and
east_data.
...
Now we can start plotting each graph separately. Firstly, the following
chunk displays the initial steps for building the west part of the
graph.
...
the y-axis in the right side or write the title. The following chunk
does all this.
...
replicate thw whole right part of the WSJ journal. The following chunk
libraries patchwork, that needs to be priorly installed, to then merge
both of them. Note that the formula is used to place the east bars (the
gold ones) below the west bars (the blue ones), as in the WSJ.
...
GDP part. The following chunk shows this process.
...
it is needed to build a negative (mirrored) variable for the EAST block,
which is going to be in the left side of the graph. It can be done in
only one line of code, but is preferred to do it separately for reader´s
ease.
...
colors chosen are the ones displayed in the following chunk, which shows
this mentioned process.
...
the graph. The following chunk shows all this process. Note that, for
reader´ease country names have been included in the side of its block.
...
readability even more straightforward. The following chunk takes into
account this lasts steps.
...
is divided in two parts (WEST and EAST), and then merged with patchwork.
...
patchwork, including titles and captions as in the original. The
following chunk does this process and ends the modification of the graph
...
In all and that said, the following chunks replicates the new graph
building.
...
Done this, the following chunk shows the process of building of the
final GEDR graph, with the changes mentioned.
...
at 0, because of the data nature

numbers slightly differ (label \$50 in the upper graph, and number 50 in
the lower graph are not exactly aligned). Moreover, colors are flipped
in the upper graph compared with the lower one. Because of the width of
the bars, it has been decided not to plot any names, in order not to add
redundant info. However, no midpoint is reached and when looking at it,
the viewer might be confused at identifying which country expends most
on research and development.
