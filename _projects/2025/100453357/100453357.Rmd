...
In order to replicate the graph the first step is to download the required libraries.
...
The main part of the plot has data divides into two: main part which has the percentage change (colourful) and a grey plot at the bottom with the percentage change values that are close to 0 (values between -0.01 and 0.01), very or little change.

#### Tidy and divide data

We divide the data into two: "plot_data" (colourful plot) and "data_zero" (grey plot), in order to create two plots for the main part of the data. Additionally we tidy the data using "pivot_longer" so that the information is organised in columns by date.
