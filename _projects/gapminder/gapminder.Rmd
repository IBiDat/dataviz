...
![Gapminder's World Health Chart. Figure from [gapminder.org](https://gapminder.org).](gapminder.png)
...
```{r theme}
p <- p + theme_classic() + theme(panel.grid.major=element_line())
p
```
...
We are close, but several adjustments are required.
Let's remove the legend for now, add a black line around the points (see shape 21),
...
```{r final}
p <- p + annotation_custom(ggplotGrob(legend), xmin=log10(16000), ymin=10, ymax=40)
p
```
...
```{r titled, fig.asp=0.75, preview=TRUE}
p + labs(
...
- data is faceted in two columns.
...
```{r plotly, fig.asp=0.8}
plotly::ggplotly(
  p %+% gapminder + aes(frame=year) + scale_size_area(max_size=18)
)
```
