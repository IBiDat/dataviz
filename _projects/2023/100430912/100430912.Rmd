...
head(data)
data<-data |> 
  rename(entity = Entity, year = Year, amount = "Total corporate investment - inflation adjusted")
...
```{r}
data<-data |> 
  mutate(entity=factor(entity, levels=c("Merger/Acquisition", "Public Offering", "Private Investment", "Minority Stake")))
head(data)
```

### 2. BUILDING THE GRAPH & SCALES
...
g<- g + 
  scale_x_date(date_breaks="year",
               date_labels = "%Y")+
  scale_y_continuous(breaks=c(0,50000000000,100000000000,150000000000,200000000000,250000000000), labels = c("$0","$50 billion","$100 billion","$150 billion","$200 billion","$250 billion"))
```
...
```{r}
g<- g +
  geom_hline(yintercept=c(50000000000,100000000000,150000000000,200000000000,250000000000),linetype="dotted")
```
...
```{r}
g<- g+
  labs(x=NULL, y=NULL, title="Annual global corporate investment in artificial intelligence, by type", 
...
```{r}
data<-data |> mutate(entity=factor(entity, levels=c("Private Investment", "Merger/Acquisition", "Public Offering", "Minority Stake"))) |> 
  arrange(data)
```
...
```{r}
p<- p +
  geom_hline(yintercept=c(0,20000000000,40000000000,60000000000,80000000000,100000000000,120000000000),linetype="dotted",color="#5b5b5b")
