...
p <-   
  ...
  labs(title = "Distribution of population between different poverty\nthresholds, World, 1990 to 2019",
       ...
       caption = "Data source: World Bank Poverty and Inequality Platform (2022) – Learn more about this data\nNote: This data is expressed in international-$ at 2017 prices. Depending on the country and year,\nit relates to income measured after taxes and benefits, or to consumption, per capita.\nOurWorldInData.org/poverty | CC BY"   ) 
p
```

### Saving the graph

To save the graph, I tested varying width and height inputs.

```{r, echo=TRUE, eval=FALSE}
ggsave("test/replication_approach.png", width = 15, height = 8, bg = "white")
```
...
To create the plot, first the labels of the facets were created and stored in an object, which was subsequently included in the function '*facet_wrap*'. The remainder remains similar to the replication graph. The segments were adjusted to the fact that a more fine-grained y-axis provides much needed information for the interpretation of the graph.
...
a <-   
  ...
  scale_y_continuous(breaks = c(0, 500000000 , 1000000000, 1500000000), 
                     labels = c("0", "500 million", "1 billion", "1.5 billion"),
                     expand = c(0, 0)) +
  labs(x = NULL, 
       y = NULL) + 
...
```{r, echo=TRUE, eval=FALSE}
ggsave("test/alternative_approach.png", width = 8, height = 8, bg = "white")
```

### ...and make it move.

For the facet wrap, I was able to animate it. Instead of leaving the x-axis free, it is fixed in my animation, making it easier to to understand the relationship between year and amount of people pertaining to a certain category of disposable income.

```{r, echo = TRUE, eval = FALSE, fig.asp= 0.75}
a_fluid <-  
  a + 
