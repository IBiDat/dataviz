...
Some adjustments still need to be done:
...
```{r slight additional changes}

# The plot, with this slight change}
p <-
  data |>
  filter(Year == 2019) |>
  ggplot() +   
...
The look is a lot cleaner, with softer edges. Let's finally try to :
...
p <-
  ...
  theme(
    plot.title = element_text(
      family = 'merriweather',                                    
      face = 'bold', # face added
      size = 15,                                   
      hjust = 0.1, # hjust added
      margin = margin(t = -10)
      ),
    plot.subtitle = element_text(
      hjust = 0.11, # hjust added
      size = 11), # size added
    plot.caption = ggtext::element_markdown(
      hjust = 0.11, # hjust modified                                                 
      size = 11,# size added
      margin = margin(t = 20)
      ),          
    plot.tag.position = 'bottom',
    plot.tag = element_text(
      size = 10,                                  
      vjust = -1, # vjust added
      margin = margin(l = -301) # margin modified
      ),        
    legend.position = 'bottom',
    legend.title = element_blank(),         
    legend.text = element_text(           
...
Final tweaks? The color of the text is not black in the original plot, but this color: #5b5b5b. Also, why not adding the nice OWID logo on the top of the graph. Let's make this slight changes:
...
ggplotly(p %+% data + aes(frame=Year)) |>   
  animation_opts(frame = 0)

```

A solution that I found was to use the `{ggiraph}` package to make the plot interactive. The filtering function seems requiring some features to create interfaces that ggplot cannot provide, such as integrating the plot in a Shiny application. Therefore, we will give up this feature.
...
YEAH it works. A few things need to be modifed. "A few". First, the position of the plot in the panel is not good. After a few iterations, I noticed that this was just a matter of playing on the margins already existing in our ggplot. Similarly, other `theme()` elements need to be adjusted.

```{r ggiraph plot adjustments}
...
p_girafe <-
  ...
  geom_sf_interactive(
    aes(        
      fill = Deaths_disc,        
      tooltip = sprintf("%s: %.3f", Entity, Deaths) # btw, this was copied from the ggiraph documentation       
      )
    ) +    
  coord_sf(crs = "+proj=robin") +
  labs(
  ...
  theme(
    plot.title = element_text(
      ...
      size = 12,                                   
      hjust = 0.1,                                   
      margin = margin(t = -100) #margin modified
      ),          
    plot.subtitle = element_text(
      color = "#5b5b5b",
      hjust = 0.1,                                      
...
Here are some improvements that we can still do: give the tooltip a better appearance and make them display the absolute number of deaths due to low physical activity.\
This interactivity with the plot will be created by adding an `options` in the `girafe()` function.

This require to create a CSS declarations for the style we want for the elements that are hovered, and pass these declarations to the `css` argument of the `hover_opt()` functions and functions related. These functions enable us to tailor the effects of hovering parts of the graph with our mouse. For sake of efficiency and because of my lack f knowledge in CSS, the declarations are generated by AI and adjusted if needed.

```{r tooltip customization}
...
If fact, one of the cool features that it has is that it provides a filter that enables you to select which countries you want to see on the graph and greatly facilitates the comparison of evolutions between countries. However, I couldn't find ways to provide this feature other than placing the graph into a Shiny Application. Since I really don't know anything about Shiny, I'll focus on a graph of the 10 countries with most Deaths by year.

## Coordinate and axes

Unlike the map, of graph has axis. These axes have:
...
```{r adding the data to line graph}

p_line <-
  data |>
  filter(Year == 2019) |> 
  slice_max(n = 10, order_by = Deaths) |> # Now, our data is filtered
  ggplot(aes(
    x = Year, 
    y = Deaths)
    ) +  
...
Before annotating the plot as the original one, we need to make room for the annotations, which will be place between the last data point of a line and the right edge of the plot. Then, we can add the annotations thanks to the `{ggrepel}` package.

```{r annotation first try}
...
The `{ggiraph}` version follows obviously the same logic as the map we previously created. We're therefore appending \_interactive to the `geom_line()` and to the `geom_point()` geoms.

```{r ggiraph line graph}
...
I actually found a great tutorial to do so: <https://uncharteddata.netlify.app/posts/2022-09-30-interactive-tooltip-tables/>
...
```{r crafting a function for the tables}
...
```{r running the table function and adding the column with tables}
...
Therefore, the alternative graphs proposed are replicates of the original, but showing the data as percentage of the population that died from low physical activity.
...
data_alt <-    
  data |> 
  mutate(
    Percentage = round((Deaths/Population)*100, digits = 3),
    .after = Deaths_disc
...
```{r interaction version of the alternative map}
...
p_map_alt_girafe <-   
  ...
  geom_sf_interactive(aes(        
    ...
    tooltip = 
      paste0(sprintf("<b>%s</b>: %.3f", Entity, Percentage), "%\nNumber of Deaths: ", round(Deaths)),
    data_id = Entity
  )) +
  coord_sf(crs = "+proj=robin") +
  labs(title = plot_title,        
...
```{r attempt to use percentage with the line graph}
...
p_line_alt <-    
  ...
  geom_vline(xintercept = 0) +
  # The reference lines will not be the same. As the max percentages between the countries plotted are around 0.03%, let's set a horizontal reference line each 0.01%   
  geom_hline(
    yintercept = seq(from = 0, to = 0.04, by = 0.01),
    linetype = 'dashed',            
    color = '#dddddd'
  ) +
  ...
  scale_y_continuous(breaks = seq(from = 0, to = 0.04, by = 0.01), # Also Adjusted
                     expand = expansion(mult = c(0, 0.1)
                     )
  ) +
  labs(
...
```{r creating two slope charts}

plot_subtitle_alt <- "Estimated annual percentage of the populations with deaths attributed to low physical activity."  
...
+   theme_classic() +   
  ...
  geom_text_repel(
    aes(
      label = paste0(Percentage, "%", " ",Entity),                       
      color = Entity),
    data = data_alt |>
      filter(Year == 2019) |>                        
  ...
  geom_text_repel(
    aes(
      label = paste0(Entity, " ", Percentage, "%"),
      color = Entity),
    data = data_alt |>
      filter(Year == 1990) |>
      filter(Entity %in% countries_top_percentage_1990),
  ...
  geom_vline(xintercept = 0) +
  geom_hline(
    yintercept = seq(from = 0.02, to = 0.06, by = 0.01),
    linetype = 'dashed',
    color = '#dddddd'
  )
...
p_slope_2019 <-    
  ...
  geom_vline(xintercept = 0) +
  geom_hline(yintercept = seq(from = 0.02, to = 0.06, by = 0.01),
             linetype = 'dashed',
             color = '#dddddd'
  )   
...
```{r adding the color palette to the slope charts}
...
#distill package 
#Figure Layout 
#For making the plot spread over more space

```

Remaining:

-   Modify titles and subtitles on every plot

-   Add OWID logo

-   changer le style des ptits tableaux dans la tooltip

-   Dans le line graph, mettre les 5 plus gros et 5 plus petits aussi, avec rouge les plus gros et bleu les plus petits

-   Changer la color scale pour qu'elle soit plus représentative de la réalité
