...
The most difficult aspect of putting together the plot was collecting and transforming the data. Instead of using website link that Nathan Yau used to download his data, I went to the [U.S. Bureau of Labor Statistics](https://www.bls.gov/tus/) website, which has all the data for the American Time Use Survey. 

After combining three datasets downloaded from the BLS.gov site using `tidyverse` joins and transformations, I exported the data to a csv file. You can see an overview of the data below. 
...
I added annotations using `geom_text` and set the color scale manually.
...
```{r knitr-1, echo=FALSE, fig.cap="Replication", layout = 'l-body-outset', preview =TRUE, fig.align='center'}

knitr::include_graphics("Replication_plot.png")
```

```{r replication-plot, eval=FALSE, echo=FALSE}
plt <- data |> 
  ggplot(aes(x = TEAGE, y = p, fill = category)) +
  geom_col(
    width = .8, 
...
For the enhanced version, I replaced the color scheme to represent the magnitude of each bar/column in percentage points instead of the category. 
...
```{r enhanced-plot, echo=FALSE, eval=FALSE}
plt_2 <- data |> 
  ggplot(
...
```{r knitr-2, echo=FALSE, fig.cap="Enhanced version", layout = 'l-body-outset', fig.align='center'}
