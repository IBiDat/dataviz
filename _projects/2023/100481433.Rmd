...
#### Libraries and the data set with the frequency variables
...
```{r}
#downloaded font
#font_folder_path <- "~/Lato2OFL"
#font_import(pattern = "[.]ttf$", paths = font_folder_path, prompt = FALSE)
#loadfonts()
...
```{r}
emocracy_colors <- c( "Frec.Non.electoral.autocracies" = "#d73027", "Frec.One.party.autocracies" = "#f46d43", 
...
plot2 <- plot +
  geom_text(
    data = custom_legend,
    aes(x = x + label_distance, y = y + vertical_offset_label, label = label, color = corrected_Democracy_Type),
    size = 2.5,
    family = "Lato",
    hjust = 0
  ) +
  geom_segment(
    data = custom_legend,
    aes(x = x + segment_distance, xend = x + 1, y = y + vertical_offset_segment, yend = y + vertical_offset_segment, color = "#5b5b5b"),
    size = 0.5
  ) +
  theme(
