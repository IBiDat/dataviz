...
```{r}
data <- read.csv("C:/Users/sofia/OneDrive/Documentos/Master Computational Social Sciences/Data Visualization/Graph/Data World csv.csv", 
                 sep = ";", 
                 dec = ",")
library(ggplot2)
library(extrafont)
...
df_reduced <- data %>%
  select(Year, Frec.Non.electoral.autocracies, Frec.One.party.autocracies, Frec.Multi.party.autocracies.without.elected.executive,
         Frec.Multi.party.autocracy, Frec.Exclusive.democracies, Frec.Male.democracies, Frec.Electoral.democracies, Frec.Polyarchies) 

#FREC - refers to the frecuency of the


custom_order <- c("Frec.Non.electoral.autocracies", "Frec.One.party.autocracies", "Frec.Multi.party.autocracies.without.elected.executive",
                  "Frec.Multi.party.autocracy", "Frec.Exclusive.democracies", "Frec.Male.democracies", "Frec.Electoral.democracies", "Frec.Polyarchies")
```
...
```{r}
colores_democracias <- c( "Frec.Non.electoral.autocracies" = "#d73027", "Frec.One.party.autocracies" = "#f46d43", "Frec.Multi.party.autocracies.without.elected.executive" = "#fdae61", "Frec.Multi.party.autocracy" = "#fee090", "Frec.Exclusive.democracies" = "#e0f3f8", "Frec.Male.democracies" = "#abd9e9", "Frec.Electoral.democracies" = "#74add1", "Frec.Polyarchies" = "#4575b4" )
```
...
plot <- ggplot(df_long, aes(x = Year, y = n, fill = Democracy_Type)) +
  ...
  theme_minimal() +
  theme(
    axis.text.x = element_text(color="#5b5b5b", size = 10, family = "Lato", angle = 0, hjust = c(0.1, 0.3, 0.5, 0.5, 0.5, 1), margin = margin(t = 0)),
  ...
  ggtitle("Countries that are democracies and autocracies, World", subtitle = "Political regimes based on the classification and assessment by Skaaning et al. (2015).") +
...
To replicate the specific placement of labels and segments in the target graphic, I modified the code to permit manual adjustments for the angle, vertical, and horizontal placement of both text and segments. Thus, *distancia_segmento* makes reference to the horizontal placement of the segment, *angulo_segmento* is for the angle and *desplazamiento_vertical_segmento* is for the vertical placement.
...
plot2 <- plot +
  ...
  geom_segment(
    data = leyenda_custom,
    aes(x = x + distancia_segmento , xend = x+1+extra, y = y + desplazamiento_vertical_segmento, yend = y + angulo_segmento + desplazamiento_vertical_segmento, color = "#5b5b5b"),
    size = 0.5
  ) +
  theme(
...
plot_parallel_custom <- ggplot(df_long, aes(x = Year, y = n, color = Democracy_Type)) +
  ...
  theme(
    ...
    axis.text.x = element_text( color = "#5b5b5b", size = 9, family = "Playfair Display ExtraBold", vjust = 0 , hjust = c(0, 0, 0, 0, 0.5, 1), margin = margin(t = -9)),
  axis.text.y = element_text(color = "#5b5b5b", size = 9, family = "Playfair Display ExtraBold", vjust = 0, hjust = 0, margin = margin(r = -22 )
    )) +
  theme(
