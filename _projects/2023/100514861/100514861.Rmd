...
The Gender Pay Gap graph of 2020 as presented below is created by Eurostat which is the European office for statistics by the European Commission. The graph shows a literal gap in the earth with a woman and man standing on top of it. For them, the gap is almost invisible as the smaller gaps are closest to the surface. Eurostat collects data concerning the labour market. The Gender Pay Gap is calculated as "the difference between the average gross hourly earnings of men and women expressed as a percentage of the average gross hourly earnings of men" (Eurostat, 2021). The Gender pay gap is used as a proxy for the inequality between men and women.

![](images/Gender_pay_gap_2020.png)
...
First, the data is cleaned that is important NA's and non-useful variables are deleted and only data for the year 2020 is kept. The variables 'obs_left 'and 'obs_right' are created to be able to form the graph. The variable 'rank' is computed to order the countries from the highest pay gap to the lowest.
...
In this section the main plot is created. Note that the main plot consist of two different geoms: geom_area with geom_linerange on top. Different rectangular annotations are done to match some of the aesthetics of the original graph. Geom_text is used to annotate the graph with the country names and the values of the gender pay gap per country (obs_value). As in the original graph EU, Iceland, Norway and Switzerland with their corresponding obs_values are coloured grey whereas the rest of the countries and their obs_values is coloured black, these four countries have been excluded form the first geom_text and are later added in another geom_text with the specification of colour set as grey. Only EU is not added in the second geom_text as it asks for a different, more specific annotations and will later be added. The text colours have been generated using an online colour detector.
...
In this section the text that annotate the text is generated for the upper and lower part of the graph. The panels for the upper and lower part of the graph is computed in the next section.
...
This piece of code adds the graphics of the woman and the man in the upper part of the graph as well as the EU sign in the lower part of the graph.
...
p3<-ggdraw(p3)+
  draw_image(
    "/Users/nienke/Desktop/Data visualisation/eu.png",
    y = -0.48, x=0.445,
    scale = 0.05)
...
data6<- data2 |> 
  select(geo, TIME_PERIOD, OBS_VALUE, geo_names)

data_graph<- data2 |> 
  filter(!geo_names %in% c("CROATIA","IRELAND"))
```
...
p1021<- ggplot(europe)+
  geom_sf(aes(fill = reorder(diff_10_21_fact, diff_10_21)))+
...
p1021<-p1021+
  theme(axis.title = element_blank(),
    ...
    panel.grid.major = element_blank(),
    panel.grid.minor = element_blank(),
    plot.title = element_text(colour =  "#33201F", family = "Roboto Condensed", face = "bold"),
    plot.subtitle = element_text(family = "Roboto Condensed" ),
    legend.title = element_blank())+
  scale_fill_manual(values = c( "-8 to -6 reduction" = "#303B16",
                                  "-6 to -4" = "#61772c",
                                  "-4 to -2" = "#91B342",
                                  "-2 to 0" ="#B4CD79",
                                  "0 to 2"="#CCA3A3",
...
facet <-ggplot(data2, aes(TIME_PERIOD, OBS_VALUE)) + 
  geom_line(data = data2a, aes(TIME_PERIOD, OBS_VALUE, group= geo), colour ="#CCA3A3") +
  geom_line() + 
...
facet <-facet+
  theme( panel.background = element_blank(),
    panel.grid.major = element_blank(),
    panel.grid.minor = element_blank(),
    axis.title = element_text(family = "Roboto Condensed", colour = "#33201F"),
    ...
    strip.background=element_rect(fill="#B4CD79"),
...
p4<-p4+
  plot_annotation(
    ...
    theme= theme(plot.title =  element_text(
      family = "Roboto Condensed", colour = "#33201F", face = "bold", size = 20),
        plot.title.position = "plot",
        plot.subtitle = element_text(family = "Roboto Condensed", colour = "#33201F",margin=margin(0,0,30,0)),
