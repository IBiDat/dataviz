...
![Source: <https://www.nytimes.com/interactive/2021/climate/extreme-summer-heat.html>](Original%20chart.png)
...
![Note: This is a screen recording of the actual animation. I was unable to properly download it due to the way it was embedded in the NYT website. Apologies for the resolution.](Untitled%20video%20-%20Made%20with%20Clipchamp.gif)
...
| Note: Creating these plots also needs us to adjust the coordinates in the labels. I'm also setting a `ylim()` argument to make the plots comparable
...
```{r first plot with shadow, echo=TRUE}
plot1991.2001 <-
  plot1991.2001+ 
#Code for the 'shadow'
...
```{r shadow on all periods, echo=FALSE, fig.height=12, fig.width=12, paged.print=FALSE}
plot1980.1990 <- plot1980.1990 +
  ggpattern::geom_area_pattern(data = df.shadow,
                               stat = "identity", 
                 position = "stack", 
                 show.legend = FALSE,
...
Using the function `transition_states` from the `gganimat()`. For the animation it's actually not necessary to make an individual plot for each time period. Rather, we can define that we want our variable `period` to be what determines our transition states. The result is as follows
...
The main thing i'd like to improve in thsi plot is being able to tell the same story but without having to rely on the animation. I would like for this plot to be able to tell us that temperatures are getting hotter every decade from the first look. 
