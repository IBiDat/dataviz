...
![Energy generation by fuel type. Source: [Bloomberg](https://www.bloomberg.com/graphics/2023-opinion-apple-supply-chain-climate-change/)](Originale.PNG){.external width="100%"}
...
```{r}
#| include: false
setwd("C:/Users/carlo/Desktop/Giorgio/Computational Social Science - UC3/Data Visualization/dataviz/_projects/2023/100513254")
```
...
```{r}
uk_flag<- readPNG("UK.png")
uk_flag_x <- -0.403
uk_flag_y <- 0.803
uk_flag_width <- 0.08

thai_flag <- readPNG("Thailand.png")
thai_flag_x <- 0.21
thai_flag_y <- 0.843
thai_flag_width <- 0.08

taiwan_flag <- readPNG("Taiwan.png")
taiwan_flag_x <- 0.635
taiwan_flag_y <- 0.715
taiwan_flag_width <- 0.08

mal_flag <- readPNG("Malaysia.png")
mal_flag_x <- 0.95
mal_flag_y <- 0.41
mal_flag_width <- 0.084

india_flag <- readPNG("India.png")
india_flag_x <- 0.89
india_flag_y <- -0.397
india_flag_width <- 0.08

china_flag <- readPNG("China.png")
china_flag_x <- 0.814
china_flag_y <- -0.7
china_flag_width <- 0.077

mex_flag <- readPNG("Mexico.png")
mex_flag_x <- 0.285
mex_flag_y <- -0.85
mex_flag_width <- 0.08

sk_flag <- readPNG("South Korea.png")
sk_flag_x <- -0.588
sk_flag_y <- -0.796
sk_flag_width <- 0.094

jap_flag <- readPNG("Japan.png")
jap_flag_x <- -0.79
jap_flag_y <- -0.557
jap_flag_width <- 0.085

us_flag <- readPNG("USA.png")
us_flag_x <- -0.93
us_flag_y <- -0.19
us_flag_width <- 0.085

viet_flag <- readPNG("Vietnam.png")
viet_flag_x <- -1.012
viet_flag_y <- 0.217
viet_flag_width <- 0.08

ger_flag <- readPNG("Germany.png")
ger_flag_x <- -0.848
ger_flag_y <- 0.574
ger_flag_width <- 0.08

indonesia_flag <- readPNG("Indonesia.png")
...
ggplot(dati_long, aes(x = value, y = Country, fill = variable))
```
...
```{r fig.height=7, fig.width=11, fig.align='center'}
    ggplot(dati_long, aes(x = value, y = Country, fill = variable)) +
      ...
      theme(axis.text.x = element_text(angle = 0, vjust = 0.5, 
                                       hjust = 1, family = "Arial", face = "bold"),
            axis.text.y = element_text(family = "Arial", face = "bold.italic"),
            axis.title = element_text(family = "Arial", face = "bold"),
            axis.line = element_line(color = "black"),
            panel.grid = element_blank(),
...
```{r fig.height=7, fig.width=11, fig.align='center'}
    ggplot(dati_long, aes(x = value, y = Country, fill = variable)) +
      ...
      theme(axis.text.x = element_text(angle = 0, vjust = 0.5, hjust = 1, family = "Arial", face = "bold"),
            ...
            legend.box = "horizontal") +  # Etichette della legenda in orizzontale
      scale_x_continuous(
        labels = c("0%", "25%", "50%", "75%", "100%"),
        breaks = c(0, 25, 50, 75, 100),
        limits = c(0, 100),  # Limita l'asse x fino al valore massimo desiderato (100)
        expand = c(0, 0)
      ) +
