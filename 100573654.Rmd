...
#ggsave("p.jpg", width = 14, height = 9, dpi = "screen")
```

Attempting to manually input the main labels by looking at the original graph and placing them according to their approximate position on the x and y axis. Four vectors were created (labels, x coordinates, y coordinates and size), which were then annotated. 
```{r labels}
#vectors 
...
## Animated line chart
First of all, I want to group the bigger genres togheter, to have a cleaner look.  
```{r new-dataset}
library(stringr)
...
anim <- animate(p3, nframes = 200, fps = 20, width = 900, height = 600,  units = "px", res = 100)
anim_save("p3.gif", animation = anim)
```
